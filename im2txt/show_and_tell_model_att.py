  ...
  def build_image_embeddings(self):
    ...
    if self.mode == "inference":
      with tf.variable_scope("image_embedding") as scope:
        image_embeddings = tf.reshape(tf.expand_dims(inception_output,1), shape=[1,64,1536])
    else:
      with tf.variable_scope("image_embedding") as scope:
     # image_embeddings = tf.contrib.layers.fully_connected(
       ...
       #   activation_fn=None,
        ...
        #  scope=scope)
        image_embeddings = tf.reshape(tf.expand_dims(inception_output,1), shape=[self.config.batch_size,64,1536])
  ...
  def build_model(self):
    ...
    if self.mode == "inference":
      ...
      for t in range(63):
          ...
          los = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=targets[:,t],logits=logits)
          los = los * mask[:,t]
