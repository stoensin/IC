...
<div class="card">
  <p class="tips">
    <span class="tips_type"><icon name="captions" :scale="2"></icon></span>
    <slot name="tips_text"></slot>
    <span class="tips_love" @click="loveIt()">
      <icon name="icon-action-love" :scale="2" v-show="!pic_info.show"></icon>
      <icon class="love-active" name="icon-action-love-selected" :scale="2" v-show="pic_info.show"></icon>
    </span>
  </p>
</div>
</template>
  ...
  methods : {
    loveIt: function(){
      if (!this.pic_info.show){
        ...
        let obj = {
          id : this.pic_info.id,
          pic : this.pic_info.pic,
          captions: this.pic_info.captions,
          show : true
        }
        data.push(obj);
